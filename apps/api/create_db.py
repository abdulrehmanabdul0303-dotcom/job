...
# Convert asyncpg URL to sync psycopg2 for creation
...
try:
    ...
    if "does not exist" in str(e) or "3D000" in str(e):
        ...
        default_url_sync = sync_url.rsplit('/', 1)[0] + '/postgres'
        try:
            engine = create_engine(default_url_sync, echo=False, isolation_level="AUTOCOMMIT")
            with engine.connect() as conn:
